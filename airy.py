...
class AiryWave:
    ...
    def acceleration(self, x, z, t=0, all_points_wet=False):
        if isinstance(x, (float, int)):
            x, z = [x], [z]
        x = asarray(x, dtype=float)
        z = asarray(z, dtype=float)
        ...
        acc = zeros((x.size, 2), float) + 1
        acc[:, 0] = w ** 2 * H / 2 * cosh(k * z) / sinh(k * d) * sin(k * x - w * t)
        acc[:, 1] = - w ** 2 * H / 2 * cosh(k * z) / sinh(k * d) * cos(k * x - w * t)
