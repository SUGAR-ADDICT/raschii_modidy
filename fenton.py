...
class FentonWave:
    ...
    def acceleration(self, x, z, t=0,):
        ...
        vel=self.velocity(x,z,t)
        u=vel[0][0]
        w=vel[0][1]

        acc = zeros((x.size, 2), float)
        acc[:,0] = k**2 * (
            (
                (-c + u) * sin(J * k * (x[:,newaxis] - c * t)) * cosh(J * k * z[:,newaxis])
                + w * cos(J * k * (x[:,newaxis] - c * t))* sinh(J * k * z[:,newaxis])
            )
            * J**2 * B[1:] / cosh(J * k * self.depth)
            ).dot(J)
        
        acc[:,1] = k**2 * (
            (
                (-c + u) * cos(J * k * (x[:,newaxis] - c * t)) * sinh(J * k * z[:,newaxis])
                + w * sin(J * k * (x[:,newaxis] - c * t))* cosh(J * k * z[:,newaxis])
            )
            * J**2 * B[1:] / cosh(J * k * self.depth)
            ).dot(J)
